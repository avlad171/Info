...
int WINAPI WinMain (HINSTANCE hThisInstance, HINSTANCE hPrevInstance, LPSTR lpcmdLine, int nCmdShow)
{
    ...
    SetLayeredWindowAttributes(hwnd,RGB(0xff,0xff,0xff), 255, LWA_ALPHA);
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case WM_CREATE:
            bmpsrc = (HBITMAP)LoadImage(NULL, "b2.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
            if(!bmpsrc)
                ...
                case 'W':
                    {
                        ...
                        if(opacity > 255)
                            opacity = 255;
                        SetLayeredWindowAttributes(hwnd, RGB(0xff,0xff,0xff), opacity, LWA_ALPHA);
                    break;
                    }
                case 'S':
                    {
